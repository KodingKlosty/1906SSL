...
<div role="main" class="container">
  ...
  <form action="/data/update" method="POST">
    <h2>Edit Fruit</h2>
    <?php  
        foreach($data as $efruit){
            echo "Current Name: ".$efruit["name"]."<br>";
        }
        ?>   
        <label for="id">ID: </label> 
        <input type="text" name="id" value=<? echo $efruit["id"] ?> placeholder=<? echo $efruit["id"] ?> readonly="readonly" >
        <label for="name">Name: </label>
        <input type="text" name="name" placeholder=<? echo $efruit["name"] ?>><br>
        <button class="btn btn-success" type="submit" name="submit" value="submit">Submit</button>
        <a name='cancel' href='/data'  class='btn btn-secondary' >Cancel</a>
    </form>
  </div>
